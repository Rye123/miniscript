...
Token* token_new(TokenType type, const char* lexeme, const int lexemeLength, int lineNum, int colNum)
{
    ...
    if (type == TOKEN_NUMBER) {
        ...
        if (!literal_str) { free(lexeme_cpy); free(ret); return NULL; }
...
void token_string(char *str, const Token *token)
{
    if (token->type == TOKEN_NL)
        ...
        snprintf(str, MAX_LEXEME_SIZE, "Token: { type: %s (%d), lexeme: \"%s\", line: %d, col: %d }", TokenTypeString[token->type], token->type, token->lexeme, token->lineNum, token->colNum);
}
