...
ExecValue *value_newIdentifier(char *identifierName, Token *tokPtr) {
    ...
    name_cpy = strdup(identifierName);
...
ExecValue *value_opAdd(ExecValue *e1, ExecValue *e2) {
    ...
    if (e1->type == TYPE_NUMBER && e2->type == TYPE_NUMBER) {
        ...
        return value_newNumber(intResult, e1->tok);
    } else if (e1->type == TYPE_STRING && e2->type == TYPE_STRING) {
        s1 = e1->value.literal_str;
        s2 = e2->value.literal_str;
        resultLen = strlen(s1) + strlen(s2);
        strResult = malloc((resultLen + 1) * sizeof(char));
        strncpy(strResult, s1, strlen(s1));
        strncpy(strResult + strlen(s1), s2, strlen(s2));
        strResult[resultLen] = '\0';
        resultVal = value_newString(strResult, e1->tok);
        free(strResult);
        return resultVal;
    } else if (e1->type == TYPE_STRING && e2->type == TYPE_NUMBER) {
        s1 = e1->value.literal_str;
        s1Len = strlen(s1);
        s2Len = snprintf(NULL, 0, "%g", e2->value.literal_num);
        ...
        result[resultLen] = '\0';
        resultVal = value_newString(result, e1->tok);
        free(result);
        return resultVal;
    } else if (e1->type == TYPE_NUMBER && e2->type == TYPE_STRING) {
        s2 = e2->value.literal_str;
        s2Len = strlen(s2);
        s1Len = snprintf(NULL, 0, "%g", e1->value.literal_num);
        resultLen = s1Len + s2Len;
        result = malloc((resultLen + 1) * sizeof(char));
...
ExecValue *value_opSub(ExecValue *e1, ExecValue *e2) {
    ...
    if (e1->type == TYPE_NUMBER && e2->type == TYPE_NUMBER) {
        ...
        resultLen = s1Len - s2Len;
        resultString = malloc(sizeof(char) * (resultLen + 1));
        strncpy(resultString, s1, resultLen);
        resultString[resultLen] = '\0';
        resultVal = value_newString(resultString, e1->tok);
        free(resultString);
...
ExecValue *value_opMul(ExecValue *e1, ExecValue *e2) {
    ...
    if (e1->type == TYPE_STRING && e2->type == TYPE_NUMBER) {
        multiplier = e2->value.literal_num;
        if (multiplier < 0)
            multiplier = 0;
        ...
        newStr[resultLen] = '\0';
        resultVal = value_newString(newStr, e1->tok);
        free(newStr);
        return resultVal;
    }
...
ExecValue *value_opGt(ExecValue *e1, ExecValue *e2) {
    ...
    if (e1->type == TYPE_NUMBER && e2->type == TYPE_NUMBER) {
        ...
        s1Len = strlen(s1);
        s2Len = strlen(s2);
        smallerLen = (s1Len < s2Len) ? s1Len : s2Len;

        comparison = strncmp(s1, s2, smallerLen);
        if (comparison != 0) {
